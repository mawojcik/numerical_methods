...
FUNCTION_B_COMPONENTS = [sympy.sin(X), sympy.cos(X)]
...
def _get_coefficients_using_least_squares_method(given_points: np.ndarray, function_components: np.ndarray)\
    ...
    """
    b_vector = given_points[:, 1]   # b_vector to są wartości y
    #TODO: column stack to for
...
if sys.argv[1] == 'exercise_a':
    ...
    yValues = evaluate_function(np.array(list(map(sympy.sympify, FUNCTION_A_COMPONENTS))), FUNCTION_B_COEFFICIENTS, FUNCTION_B_XPOINTS)
    for a in range(len(yValues)):
        yValues[a] = yValues[a] + np.random.random()
    # yValues = yValues + np.random.random()
